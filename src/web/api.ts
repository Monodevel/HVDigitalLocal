...
export function guardarToken(token: string): void { sessionStorage.setItem(TOKEN_KEY, token) }
export function obtenerToken(): string | null { return sessionStorage.getItem(TOKEN_KEY) }
export function guardarSesionWeb(sesion: SesionWeb): void { sessionStorage.setItem(SESSION_KEY, JSON.stringify(sesion)) }
...
export async function cerrarSesionWeb(): Promise<void> {
  try {
    ...
    console.warn('No fue posible invalidar la sesión remota; se cerrará la sesión local.', error)
  } finally {
    limpiarToken()
  }
}
...
export async function apiJson<T>(path: string, init: RequestInit = {}): Promise<T> {
  ...
  if (!response.ok) {
    const message = typeof payload === 'string' ? payload : payload?.message || payload?.error || `Error HTTP ${response.status}`
