...
const menuItems = computed(() => dentroExpediente.value ? [
...
function cerrarMenusMoviles(): void {
  menuUsuarioMovilAbierto.value = false
  menuMasMovilAbierto.value = false
  document.documentElement.classList.remove('hv-mobile-modal-open')
}
function actualizarBloqueoMovil(): void {
  document.documentElement.classList.toggle('hv-mobile-modal-open', menuUsuarioMovilAbierto.value || menuMasMovilAbierto.value)
}
function alternarMenuUsuario(): void {
  menuMasMovilAbierto.value = false
  menuUsuarioMovilAbierto.value = !menuUsuarioMovilAbierto.value
  actualizarBloqueoMovil()
}
function alternarMenuMas(): void {
  menuUsuarioMovilAbierto.value = false
  menuMasMovilAbierto.value = !menuMasMovilAbierto.value
  actualizarBloqueoMovil()
}
function cerrarMenuUsuarioMovil(): void { menuUsuarioMovilAbierto.value = false; actualizarBloqueoMovil() }
function cerrarMenuMasMovil(): void { menuMasMovilAbierto.value = false; actualizarBloqueoMovil() }
function irAResumenExpediente(): void { cerrarMenusMoviles(); menuActivo.value = 'resumen'; pantalla.value = 'EXPEDIENTE' }
function irAHojasDeVida(): void { cerrarMenusMoviles(); menuActivo.value = periodoSeleccionado.value ? 'hojas-vida' : 'periodos'; pantalla.value = periodoSeleccionado.value ? 'CALIFICADOS' : 'PERIODOS' }
function volverJerarquico(): void {
  if (!dentroExpediente.value) return
  if (pantalla.value === 'EXPEDIENTE') irAHojasDeVida()
  else irAResumenExpediente()
}
...
async function inicializarAplicacion(): Promise<void> {
  ...
  rolUsuario.value = sesion.rol
  nombreUsuario.value = sesion.nombreMostrar?.trim() || sesion.usuario
  document.documentElement.dataset.hvRole = sesion.rol
  try { await cargarContextoAutenticado() }
  catch (e) { errorCarga.value = e instanceof Error ? e.message : String(e); pantalla.value = 'PERIODOS'; menuActivo.value = 'periodos' }
}
...
async function iniciarSesion(): Promise<void> {
  ...
  try {
    ...
    await cargarContextoAutenticado()
...
async function recargarPeriodos(): Promise<void> {
  ...
  errorCarga.value = ''
  try {
    const globalId = periodoSeleccionado.value?.globalId ?? null
    periodos.value = await listarPeriodosUi()
    if (globalId) periodoSeleccionado.value = periodos.value.find(p => p.globalId === globalId) ?? null
  } catch (e) { errorCarga.value = e instanceof Error ? e.message : String(e) }
...
async function crearPeriodoGlobal(): Promise<void> {
  ...
  try { await crearPeriodoAdmin(anio); await recargarPeriodos() }
  catch (e) { errorCarga.value = e instanceof Error ? e.message : String(e) }
  finally { administrandoPeriodo.value = false }
}
async function cambiarEstadoGlobal(periodo: PeriodoUi): Promise<void> {
  ...
  administrandoPeriodo.value = true; errorCarga.value = ''
  try { await cambiarEstadoPeriodoAdmin(periodo.globalId, nuevoEstado); await recargarPeriodos() }
  catch (e) { errorCarga.value = e instanceof Error ? e.message : String(e) }
  finally { administrandoPeriodo.value = false }
}
async function seleccionarPeriodo(periodo: PeriodoUi): Promise<void> {
  ...
  try {
    ...
    filtrosMovilesAbiertos.value = false
...
function abrirExpediente(item: CalificadoUi): void { cerrarMenusMoviles(); calificadoSeleccionado.value = item; irAResumenExpediente() }
...
async function abrirHojaVida(): Promise<void> { if (await asegurarHojaVida()) { cerrarMenusMoviles(); menuActivo.value = 'hoja-vida'; pantalla.value = 'HOJA_VIDA' } }
async function abrirAnotacion(): Promise<void> {
  if (modoSoloLectura.value) return window.alert('El período está cerrado y solo permite lectura.')
  if (await asegurarHojaVida()) { cerrarMenusMoviles(); menuActivo.value = 'anotaciones'; pantalla.value = 'ANOTACION' }
}
async function abrirEvint(numero: 1 | 2): Promise<void> {
  ...
  if (!instrumento) return window.alert(`El expediente no contiene EVINT ${numero}.`)
  cerrarMenusMoviles()
...
async function abrirDocumento(tipo: 'HC1'|'HC2'|'HAM'|'HAPSEM'): Promise<void> {
  if (await asegurarHojaVida()) { cerrarMenusMoviles(); menuActivo.value = tipo.toLowerCase(); pantalla.value = tipo }
}
async function abrirInstrumento(i: InstrumentoExpedienteDetalle): Promise<void> {
  ...
  if (['HOJA_VIDA','HOJA DE VIDA','HV'].includes(tipo)) return abrirHojaVida()
  if (tipo === 'EVINT') {
    instrumentoEvintSeleccionadoId.value = i.instrumento_id
    menuActivo.value = i.numero === 2 ? 'evint-2' : 'evint-1'
    pantalla.value = 'EVINT'
    cerrarMenusMoviles()
    return
  }
  if (['HC1','HC2','HAM','HAPSEM'].includes(tipo)) await abrirDocumento(tipo as 'HC1'|'HC2'|'HAM'|'HAPSEM')
}
function abrirResoluciones(): void { cerrarMenusMoviles(); resolucionSeleccionadaId.value = null; menuActivo.value = 'resoluciones'; pantalla.value = 'RESOLUCIONES' }
...
function abrirResolucion(id: number): void { resolucionSeleccionadaId.value = id; menuActivo.value = 'resoluciones'; pantalla.value = 'RESOLUCION_FORMULARIO' }

async function manejarMenu(id: string): Promise<void> {
  if (id === 'mas') { alternarMenuMas(); return }
  cerrarMenusMoviles()
  if (id === 'volver') { volverJerarquico(); return }
  if (id === 'hojas-vida') { irAHojasDeVida(); return }
  menuActivo.value = id
  if (id === 'periodos') { await recargarPeriodos(); pantalla.value = 'PERIODOS' }
  else if (id === 'notas') { if (!periodoSeleccionado.value) { menuActivo.value = 'periodos'; return window.alert('Seleccione primero un período.') } pantalla.value = 'NOTAS' }
  else if (id === 'configuracion') pantalla.value = 'CONFIGURACION'
  else if (id === 'perfil') pantalla.value = 'PERFIL'
  else if (id === 'admin-usuarios' && esAdmin.value) pantalla.value = 'ADMIN_USUARIOS'
  else if (id === 'designacion') {
    if (!periodoSeleccionado.value) { menuActivo.value = 'periodos'; return window.alert('Seleccione primero un período.') }
    if (modoSoloLectura.value) { menuActivo.value = 'hojas-vida'; return window.alert('No se puede agregar personal a un período cerrado.') }
    pantalla.value = 'DESIGNACION'
  }
  else if (id === 'resumen') irAResumenExpediente()
  ...
  <div v-else class="hv-app-shell hv-app-shell-with-status">
    ...
    <section class="hv-workspace">
      <header class="hv-topbar">
        <button v-if="dentroExpediente" type="button" class="hv-mobile-only hv-mobile-back" :aria-label="pantalla === 'EXPEDIENTE' ? 'Volver a Hojas de Vida' : 'Volver al resumen'" @click="volverJerarquico"><i class="pi pi-chevron-left" /></button>
      ...
      <div v-else-if="pantalla === 'EXPEDIENTE' && expedienteSeleccionadoId" class="hv-module-host"><ExpedienteDetalleView :expediente-id="expedienteSeleccionadoId" @volver="irAHojasDeVida" @abrir-instrumento="abrirInstrumento" @abrir-evint="id => { instrumentoEvintSeleccionadoId=id; menuActivo='evint-1'; pantalla='EVINT' }" @abrir-hoja-vida="abrirHojaVida" @nueva-anotacion="abrirAnotacion" /></div>
      <div v-else-if="pantalla === 'HOJA_VIDA' && hojaVidaSeleccionadaId" class="hv-module-host"><HojaVidaView :hoja-vida-id="hojaVidaSeleccionadaId" @volver="irAResumenExpediente" @nueva-anotacion="abrirAnotacion" /></div>
      ...
      <div v-else-if="pantalla === 'EVINT' && instrumentoEvintSeleccionadoId" class="hv-module-host"><EvintView :instrumento-id="instrumentoEvintSeleccionadoId" @volver="irAResumenExpediente" /></div>
      <div v-else-if="pantalla === 'HC1' && hojaVidaSeleccionadaId" class="hv-module-host"><Hc1View :hoja-vida-id="hojaVidaSeleccionadaId" @volver="irAResumenExpediente" /></div>
      <div v-else-if="pantalla === 'HC2' && hojaVidaSeleccionadaId" class="hv-module-host"><Hc2View :hoja-vida-id="hojaVidaSeleccionadaId" @volver="irAResumenExpediente" /></div>
      <div v-else-if="pantalla === 'HAM' && hojaVidaSeleccionadaId" class="hv-module-host"><HamView :hoja-vida-id="hojaVidaSeleccionadaId" @volver="irAResumenExpediente" /></div>
      <div v-else-if="pantalla === 'HAPSEM' && hojaVidaSeleccionadaId" class="hv-module-host"><HapsemView :hoja-vida-id="hojaVidaSeleccionadaId" @volver="irAResumenExpediente" /></div>
      ...
      <div v-else-if="pantalla === 'CONFIGURACION'" class="hv-module-host"><ConfiguracionSistemaView @volver="irAHojasDeVida" /></div>
      <div v-else-if="pantalla === 'ADMIN_USUARIOS' && esAdmin" class="hv-module-host"><UsuariosView /></div>
    </section>

    <nav class="hv-mobile-only hv-mobile-nav" aria-label="Navegación principal"><button v-for="item in menuMovilPrincipal" :key="item.id" type="button" :class="{ active: item.id === 'mas' ? menuMasMovilAbierto || menuSecundarioActivo : menuActivo === item.id }" @click="manejarMenu(item.id)"><i :class="item.icon" /><span>{{ item.label }}</span></button></nav>

    <div v-if="menuUsuarioMovilAbierto" class="hv-mobile-only hv-mobile-popover-backdrop" @click="cerrarMenuUsuarioMovil"><section class="hv-mobile-user-menu" @click.stop><header><span class="hv-mobile-user-avatar">{{ inicialesUsuario }}</span><div><strong>{{ nombreUsuario || usuario }}</strong><small>{{ rolUsuario === 'ADMIN' ? 'Administrador' : 'Calificador' }}</small></div></header><button type="button" @click="manejarMenu('perfil')"><i class="pi pi-user" /><span>Mi perfil</span><i class="pi pi-chevron-right" /></button><button type="button" @click="manejarMenu('configuracion')"><i class="pi pi-cog" /><span>Configuración</span><i class="pi pi-chevron-right" /></button><button v-if="esAdmin" type="button" @click="manejarMenu('admin-usuarios')"><i class="pi pi-users" /><span>Usuarios y accesos</span><i class="pi pi-chevron-right" /></button><button type="button" class="danger" @click="cerrarSesion"><i class="pi pi-sign-out" /><span>Cerrar sesión</span></button></section></div>

    <div v-if="menuMasMovilAbierto && dentroExpediente" class="hv-mobile-only hv-mobile-sheet-backdrop" @click="cerrarMenuMasMovil"><section class="hv-mobile-more-sheet" @click.stop><header><div><strong>Instrumentos</strong><small>{{ calificadoSeleccionado?.grado }} {{ calificadoSeleccionado?.nombre }}</small></div><button type="button" aria-label="Cerrar" @click="cerrarMenuMasMovil"><i class="pi pi-times" /></button></header><div class="hv-mobile-more-grid"><button v-for="item in menuMovilMas" :key="item.id" type="button" :class="{ active: menuActivo === item.id }" @click="manejarMenu(item.id)"><i :class="item.icon" /><span>{{ item.label }}</span></button></div></section></div>
