...
export async function obtenerEstadoBasesDatos():Promise<DatabaseStatus[]>{
 if(!esAdmin())return[]
 const bases=await apiJson<DatabaseStatus[]>('/backup/status');const maria=bases[0];if(!maria)return[]
 return[{...maria,name:'hvdigital.db',path:`${maria.path} · datos operacionales`},{...maria,name:'catalog.db',path:`${maria.path} · catálogos institucionales`}]
}
...
export function formatearFechaUnix(value:number|null|undefined):string{if(!value)return'Administrado por MariaDB';return new Intl.DateTimeFormat('es-CL',{dateStyle:'medium',timeStyle:'short'}).format(new Date(value*1000))}
