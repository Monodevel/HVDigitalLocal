...
/** El esquema de resoluciones externas se administra en migraciones MariaDB. */
...
export async function crearResolucionExterna(entrada: CrearResolucionExternaInput): Promise<number> {
  ...
  const hoja = await db.select<Array<{ total: number }>>(
    'SELECT COUNT(*) AS total FROM hojas_vida WHERE id = $1 AND persona_id = $2',
    [entrada.hojaVidaId, entrada.personaId],
  )
  if (Number(hoja[0]?.total ?? 0) !== 1) throw new Error('La Hoja de Vida no corresponde al personal seleccionado.')
  ...
  if (Number(duplicadas[0]?.total ?? 0) > 0) {
    throw new Error('Esta resolución externa ya se encuentra registrada para el personal seleccionado.')
  }

  const correlativos = await db.select<Array<{ siguiente: number }>>(`
    SELECT CASE WHEN MIN(correlativo) IS NULL OR MIN(correlativo) >= 0
      THEN -1 ELSE MIN(correlativo) - 1 END AS siguiente
    FROM resoluciones_documentales
