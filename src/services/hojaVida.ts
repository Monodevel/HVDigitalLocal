...
function centecimasAVisual(valor: number): string {
  const signo = valor > 0 ? '+' : ''
  return `${signo}${(valor / 100).toFixed(2).replace('.', ',')}`
}

function validarHojaVidaId(hojaVidaId: number): void {
  if (!Number.isInteger(hojaVidaId) || hojaVidaId <= 0) {
    throw new Error('El identificador de la Hoja de Vida no es válido.')
  }
}
...
Promise<PersonaConHojaVidaAbierta[]> {
  ...
  const filas = await db.select<Array<{
    ...
    FROM hojas_vida hv
    INNER JOIN personas p ON p.id = hv.persona_id
  ...
  return filas.map(fila => {
    ...
    const grado = fila.grado_calidad_abreviatura?.trim() ?? ''

    return {
      ...fila,
      persona_nombre_completo: nombreCompleto,
      nombre_completo: nombreCompleto,
      etiqueta: [grado, nombreCompleto, fila.run ? `RUN ${fila.run}` : '']
        .filter(Boolean)
        .join(' · '),
    }
  })
...
export async function listarHojasVidaAbiertasPorPersona(
  ...
  const filas = await db.select<Array<{
    ...
    LEFT JOIN grados g ON g.id = hv.grado_id_inicio
    LEFT JOIN calidades_personal cp ON cp.id = hv.calidad_personal_id_inicio
    INNER JOIN categorias_personal c ON c.id = hv.categoria_id
    INNER JOIN periodos per ON per.id = hv.periodo_id
    WHERE hv.persona_id = $1 AND hv.estado = 'abierta'
    ORDER BY hv.fecha_inicio DESC, hv.id DESC
  `, [personaId])
  ...
  return filas.map(fila => {
    ...
    return {
      ...
      grado_calidad_abreviatura: grado,
    }
  })
}
...
export async function obtenerHojaVidaResumen(
  ...
  const db = await obtenerBaseDatos()
  const filas = await db.select<HojaVidaResumen[]>(`
    SELECT *
  ...
  return {
    ...resumen,
    total_anotaciones: Number(resumen.total_anotaciones ?? 0),
    total_meritos: Number(resumen.total_meritos ?? 0),
    total_demeritos: Number(resumen.total_demeritos ?? 0),
    total_neutras: Number(resumen.total_neutras ?? 0),
    total_borradores: Number(resumen.total_borradores ?? 0),
    puntaje_acumulado_centecimas: Number(
      resumen.puntaje_acumulado_centecimas ?? 0,
    ),
    ...
    puntaje_acumulado_visual: centecimasAVisual(
      Number(resumen.puntaje_acumulado_centecimas ?? 0),
    ),
  }
}
...
export async function listarBorradoresHojaVida(
  ...
  return db.select<BorradorHojaVida[]>(`
    ...
    ORDER BY fecha_anotacion ASC, borrador_id ASC
  `, [hojaVidaId])
}
...
export async function listarAnotacionesHojaVida(
  ...
  validarHojaVidaId(hojaVidaId)
  const db = await obtenerBaseDatos()
  const filas = await db.select<AnotacionHojaVida[]>(`
    SELECT *
    FROM vw_anotaciones_hoja_vida
    WHERE hoja_vida_id = $1
      AND estado = 'estampada'
    ORDER BY fecha_anotacion ASC, anotacion_id ASC
  `, [hojaVidaId])
  ...
  return filas.map(fila => ({
    ...
    computa_calificacion: Number(fila.computa_calificacion),
    requiere_resolucion: Number(fila.requiere_resolucion),
    puntaje_centecimas:
...
export async function anularBorradorHojaVida(
  ...
  const resultado = await db.execute(`
    ...
    WHERE id = $1 AND estado IN ('borrador', 'validado')
  `, [borradorId])

  if (resultado.rowsAffected === 0) {
    throw new Error('El borrador no existe o ya no puede anularse.')
  }
}
...
export async function anularAnotacionHojaVida(
  ...
  const resultado = await db.execute(`
    UPDATE anotaciones
    SET estado = 'anulada'
    WHERE id = $1 AND estado = 'estampada'
  `, [anotacionId])

  if (resultado.rowsAffected === 0) {
    throw new Error('La anotación no existe o ya no puede anularse.')
  }
}

export async function obtenerUnidadSistema(): Promise<string> {
  const db = await obtenerBaseDatos()
